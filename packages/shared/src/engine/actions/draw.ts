...
export function handleDrawTile(state: GameState, action: DrawTileAction): ActionResult {
  ...
  if (!tile) throw new Error("handleDrawTile: wall unexpectedly empty after validation");
...
export function advanceTurn(state: GameState): void {
  ...
  if (!currentPlayer)
    throw new Error(`advanceTurn: no player found for currentTurn '${state.currentTurn}'`);
