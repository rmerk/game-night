...
describe("loadCard", () => {
  ...
  test("all loaded hands have exactly 14 tiles", () => {
    ...
    for (const cat of card.categories) {
      for (const hand of cat.hands) {
        const total = hand.groups.reduce((sum, g) => sum + GROUP_SIZES[g.type], 0);
  ...
  test("all loaded hands have non-empty group arrays", () => {
    ...
    for (const cat of card.categories) {
      for (const hand of cat.hands) {
        expect(hand.groups.length, `Hand ${hand.id} has empty groups`).toBeGreaterThan(0);
      }
    }
  });
    ...
    for (const cat of card.categories) {
      for (const hand of cat.hands) {
        for (const group of hand.groups) {
          expect(validTypes, `Hand ${hand.id} has invalid group type: ${group.type}`).toContain(
            group.type,
          );
        }
      }
  ...
  test("non-news/dragon_set groups have tile field", () => {
    ...
    for (const cat of card.categories) {
      for (const hand of cat.hands) {
        for (const group of hand.groups) {
          if (group.type !== "news" && group.type !== "dragon_set") {
            expect(group.tile, `Hand ${hand.id}: ${group.type} must have tile field`).toBeDefined();
          }
        }
      }
    }
...
describe("validateAndParse error handling", () => {
  ...
  test("throws for missing or invalid year", () => {
    ...
    expect(() => validateAndParse({ year: "2026", categories: [] })).toThrow(
  ...
  test("throws for invalid category", () => {
    expect(() => validateAndParse({ year: 2026, categories: ["bad"] })).toThrow(
      "Category at index 0 must be an object",
    );
    expect(() => validateAndParse({ year: 2026, categories: [{ hands: [] }] })).toThrow(
      'non-empty "name" string',
    );
    expect(() => validateAndParse({ year: 2026, categories: [{ name: "T", hands: [] }] })).toThrow(
      'non-empty "hands" array',
    );
  });
  ...
  test("throws for invalid hand structure", () => {
    ...
    expect(() => validateAndParse(wrap([{ points: 25, exposure: "X", groups: [] }]))).toThrow(
      'non-empty "id" string',
    );
    expect(() =>
      validateAndParse(wrap([{ id: "x", points: 0, exposure: "X", groups: [] }])),
    ).toThrow('"points" must be a positive number');
    expect(() =>
      validateAndParse(wrap([{ id: "x", points: 25, exposure: "Z", groups: [] }])),
    ).toThrow('"exposure" must be "X" or "C"');
    expect(() =>
      validateAndParse(wrap([{ id: "x", points: 25, exposure: "X", groups: [] }])),
    ).toThrow('non-empty "groups" array');
  });
  ...
  test("throws for invalid group type", () => {
    ...
    expect(() => validateAndParse(wrap([{ type: "bad", jokerEligible: false }]))).toThrow(
      '"type" must be one of',
    );
  });
  ...
  test("throws for incorrect joker eligibility", () => {
    ...
    expect(() =>
      validateAndParse(
        wrap([{ type: "pair", tile: { color: "A", value: 1 }, jokerEligible: true }]),
      ),
    ).toThrow("pair (size 2) cannot be joker eligible");
    expect(() =>
      validateAndParse(
        wrap([{ type: "pung", tile: { color: "A", value: 1 }, jokerEligible: false }]),
      ),
  ...
  test("throws for news/dragon_set with tile field", () => {
    ...
    expect(() =>
      validateAndParse(wrap([{ type: "news", tile: { category: "wind" }, jokerEligible: true }])),
    ).toThrow('news must not have a "tile" field');
    expect(() =>
      validateAndParse(
        wrap([{ type: "dragon_set", tile: { category: "dragon" }, jokerEligible: true }]),
      ),
  ...
  test("throws for non-implicit group without tile field", () => {
    ...
    expect(() => validateAndParse(wrap([{ type: "kong", jokerEligible: true }]))).toThrow(
      'kong must have a "tile" field',
    );
  });
  ...
  test("throws for invalid tile requirement category", () => {
    ...
    expect(() =>
      validateAndParse(
        wrap([{ type: "pung", tile: { category: "invalid" }, jokerEligible: true }]),
      ),
