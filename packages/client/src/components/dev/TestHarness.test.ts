...
describe('TestHarness – game initialisation (AC 1, 2)', () => {
  ...
  it('starts with empty discard pools for all players', () => {
    const h = makeHarness()
    h.initGame()
    Object.values(h.getState().players).forEach((p) =>
      expect(p.discardPool).toHaveLength(0),
    )
  })
})
...
describe('TestHarness – draw action (AC 3)', () => {
  ...
  beforeEach(() => {
    ...
    h.dispatch({ type: 'DISCARD_TILE', playerId: east.id, tileId: h.getDiscardableTileId(east.id) })
  })
...
describe('TestHarness – dev-only gating (AC 6)', () => {
  ...
  it('dev routes array is populated when DEV is true', () => {
    ...
    const devRoutes = isDev
      ? [{ path: '/dev/harness', name: 'dev-harness' }]
      : []
  ...
  it('dev routes array is empty when DEV is false', () => {
    ...
    const devRoutes = false ? [{ path: '/dev/harness', name: 'dev-harness' }] : []
...
describe('TestHarness – component rendering', () => {
  ...
  it('shows draw button when in draw phase', async () => {
    ...
    // Initial state: East is in discard phase, so no draw button
    const drawBtnBefore = wrapper.findAll('button').find((b) => b.text().includes('Draw for'))

    // Discard a tile to advance to next player's draw phase
    const rackButtons = wrapper.findAll('button').filter(
      (b) => !b.text().includes('Draw') && !b.text().includes('New Game') && !b.attributes('disabled'),
    )
  ...
  it('clicking a rack tile discards it and updates the display', async () => {
    ...
    const clickableTiles = wrapper.findAll('button').filter(
      (b) => !b.text().includes('Draw') && !b.text().includes('New Game') && !b.attributes('disabled'),
    )
    expect(clickableTiles.length).toBeGreaterThan(0)

    const tileText = clickableTiles[0]!.text()
