...
- **Code review pass 2 (2026-04-04):** Found **scoreboard phase** gap: desktop chat/NMJL toggles stayed visible while `SlideInReferencePanels` / `MobileBottomBar` were `v-if="!isScoreboardPhase"`, so `openChat()` could leave `activePanel === 'chat'` with **no panel DOM** (broken UX, AC9 “all game phases”). **Fix:** always mount slide-in panels + mobile bar + chat shell anchor; add `scoreboardChatFocusReturnRef` for Escape when there is no action zone (AC6).
...
- **2026-04-04 (code review pass 2):** Scoreboard-phase chat UI + Escape focus sink fix in `GameTable.vue`; regression gate re-run.
